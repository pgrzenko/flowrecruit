...
import type { Hex as HexModel, HexPayload } from "@/state/types";
...
/** Mapa pozycji { [hexId]: { cx, cy } } bazująca na axial coords. */
function useHexPositions(
  ...
  for (const hex of hexes) {
    ...
    const ring = Number(hex.ring);
    const axials = ringAxials[ring] || [];
    // id w formacie r{ring}-{idx}, np. r2-7
    let idx = 0;
...
export const Stage: React.FC = () => {
  ...
  const layout = useMemo(
    () => useHexPositions(hexes, width, height),
    [hexes, width, height]
  );
